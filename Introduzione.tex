...
Gli sviluppatori sono soliti utilizzare messaggi di tracciamento (\textit{log}) per stampare a video o salvare in \textit{file} stati significativi delle applicazioni. I messaggi più frequenti riportati nei \textit{log} sono quelli relativi a situazioni di errore (\textit{Exception} e \textit{Stack Trace}).

L'approccio comune alla creazione e gestione dei \textit{log} presenta la criticità specifica della \textit{località}, poiché tipicamente questi \textit{file} vengono salvati nella stessa macchina sulla quale sta operando l'applicazione.
...
Nel primo capitolo tratteremo approfonditamente la tematica dei \textit{log}, i contesti nei quali essi vengono utilizzati e le problematiche legate alla gestione di questo tipo di soluzione di tracciamento. Vedremo anche come utilizzare i \textit{log} per ottenere informazioni di supporto alla \textit{business intelligence}. 
