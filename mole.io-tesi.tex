...
%% Formato proposto dall'università
...
\clearpage
	...
	\input{Architettura_del_sistema}
	\clearpage
		\subsection{Command Query Responsibility Segregation}\label{Command_query_responsibility_segregation}
		\input{Command_query_responsibility_segregation}
		\clearpage
		\subsection{Mole}\label{mole}
		\input{mole}
		\clearpage
			%\subsubsection{I Denormalizzatori}\label{I_denormalizzatori}
			%\input{I_denormalizzatori}
			%\clearpage
		\subsection{Mole Suit}\label{mole-suit}
		\input{mole-suit}
		\clearpage
			%\subsubsection{I Plugin e gli Widget}\label{I_plugin_e_gli_widget}
			%\input{I_plugin_e_gli_widget}
			%\clearpage
	\section{Autenticazione degli Utenti}\label{Autenticazione_degli_utenti}
	\input{Autenticazione_degli_utenti}
	\clearpage
	\section{Scalabilità e Affidabilità}\label{Scalabilita_e_affidabilita}
	\input{Scalabilita_e_affidabilita}
	\clearpage
	%\section{Problematiche di Sviluppo}\label{Problematiche_di_sviluppo}
	%\input{Problematiche_di_sviluppo}
	%\clearpage

\chapter{Configurazioni e Benchmark}\label{Configurazioni_e_benchmark}
\input{Configurazioni_e_benchmark}
\clearpage
...
\input{Conclusioni_e_sviluppi_futuri}
\clearpage
...
%- cos'è nodejs (storia, peculiarità, async, pesca da presentazione lucio)
...
%- sicurezza (oauth)
%- scalabilità (+ server con load balancer? cloud? rabbit, disaccoppiamento dei servizi)
