...
La peculiarità di questo servizio \cite{website:Log.io} è l'aspetto \textit{realtime}, infatti esso permette di ottenere in tempo reale i log in arrivo dalle applicazioni monitorate.
...
Tra le applicazioni a pagamento censite, questa sembra essere la più completa \cite{website:Rollbar}, permette infatti di registrare sia messaggi di errore sia messaggi generici.
...
Ogni qual volta un sistema viene pubblicato in produzione, è possibile inviare una notifica a Rollbar. Questo permette agli analisti di correlare gli errori in arrivo con la specifica versione dell'applicazione dalla quale essi sono stati generati. 
...
E' un tool molto orientato agli amministratori di rete \cite{website:Papertrail}. Nei sitemi \textit{unix-like}, il comando \verb|tail|, permette di stampare in \textit{console} in tempo reale le ultime righe aggiunte ad un file utilizzato da un processo. Analogamente, lo stesso comando in Papertrail, permette di ottenere la coda degli ultimi messaggi ricevuti in tempo reale con una operazione chiamata \textit{Tail and Search}. Papertrail salva periodicamente i log ricevuti e permette di scaricarli, rendendoli così disponibili per eventuali elaborazioni successive.

A livello di analisi automatizzata, questo tool, esegue una classificazione dei messaggi ricevuti estrapolando autonomamente la chiave di aggregazione dal formato stesso del messaggio.
...
E' possibile utilizzare diversi criteri per il filtraggio dei messaggi di errore. Tra questi è di particolare interesse il filtro temporale, è infatti possibile selezionare un intervallo di tempo e ottenere l'elenco dei messaggi ricevuti in quel momento.
...
Nella analisi comparativa in corso è necessario inserire anche questa architettura composta da tre applicazioni che collaborano sinergicamente al fine di creare uno strumento flessibile e completo per la raccolta, l'archiviazione, la ricerca e l'analisi dei log.
...
Nella tabella \ref{tabella-comparativa delle-funzionalita} sono riportate schematicamente le principali caratteristiche analizzate nelle soluzioni, per la centralizzazione dei log, prese in esame.

\begin{sidewaystable}
\begin{tabular}{l|c|c|c|c|c}
& \textbf{Airbrake} & \textbf{Log.io} & \textbf{Rollbar}& \textbf{Papertrail} & \textbf{Fluentd, ElasticSearch, Kibana}\\ 
\hline 
...
\end{tabular} 
\caption{Tabella comparativa delle funzionalità}\label{tabella-comparativa delle-funzionalita}
\end{sidewaystable}