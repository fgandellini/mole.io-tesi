Il \textit{deploy} è tecnicamente la fase di installazione dell'applicazione web su un server, sia esso di test o di produzione.
...
Docker è un sistema open-source per l'automatizzazione del processo di deploy di applicazioni. Per eseguire questo compito si avvale di \textit{container}, veri e propri contenitori virtuali di applicazioni con le relative dipendenze, in grado di essere eseguiti su server Linux.\\
...
L'utilizzo di container offre svariati vantaggi, il principale è la portabilità delle applicazioni su diversi server fisici o servizi \textit{cloud} pubblici e privati.\\
...
\`{E} stata realizzata una macchina virtuale con sistema operativo Ubuntu Server, all'interno della quale è stato installato Dokku, un software in grado di trasformare il sistema in un server di tipo PaaS.
...
\end{verbatim}
Con questo comando, sfruttando \verb|git|, il sistema di \textit{versioning} del codice, è possibile eseguire una operazione di \verb|push| del proprio \textit{branch} \verb|master| sul server \verb|test|.
